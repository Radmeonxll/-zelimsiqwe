...
`)
  ...
  const newnick = `${member.user.username.includes(Guild.Secondary_Tag) ? Guild.Secondary_Tag : (Guild.Secondary_Tag ? Guild.Secondary_Tag : (Guild.Secondary_Tag))} ${nick} ${Guild.Tag}`;
...
${member.toString()} üyesinin ismi başarıyla "${nick} ${Guild.Tag}" ismine değiştirildi. Bu üye daha önce şu isimlerle kayıt olmuş:\n
${this.client.no} Kişinin Toplamda ${registerModel?.userNames?.length ?? 0} isim kayıtı bulundu.
${registerModel?.userNames?.map(x => `\`• ${x.nick}\` (${x.type.replace(`Erkek`, `<@&${Role.Register.Man[0]}>`).replace(`Kız`, `<@&${Role.Register.Woman[0]}>`)})`).slice(0, 10).join("\n ")}\n
Kişinin önceki isimlerine \`!isimler @üye\` komutuyla bakarak kayıt işlemini gerçekleştirmeniz önerilir.` 
: `
${member.toString()} üyesinin ismi başarıyla "${nick} ${Guild.Tag}" ismine değiştirildi.`}
`);
  ...
  if (onay.size < 1) {
    ...
    this.client.channels.cache.get(Log.Zade.Nick).send(new MessageEmbed()
    .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true })).setColor("BLACK").setDescription(`
  ...
  if (onayContent.includes(".e")) {
    ...
    this.client.channels.cache.get(Log.Register2.Log).send(new MessageEmbed()
    .setColor("BLACK")
    .setAuthor(message.author.tag, message.author.avatarURL({ dynamic: true }))
    .setDescription(`
