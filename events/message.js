...
module.exports = class {
  ...
  async run(message) {
    ...
    this.client.channels.cache.get(Log.Komut.Log).send(new Discord.MessageEmbed().setColor("BLACK").setTimestamp().setAuthor(message.author.tag, message.author.avatarURL()).setDescription(`${message.author.tag} - (${message.author.id}) komut kullandı "${cmd.help.name}" kullandığı kanal ${message.channel.name} - ${message.channel.id}`))
 }
};
